...
# ---------------------------------------------------------------------------
# Unauthorized Widening Deltas check
# ---------------------------------------------------------------------------

from bernstein.core.security.audit_chain import get_unauthorized_widening_deltas

def test_get_unauthorized_widening_deltas_empty_when_no_deltas(tmp_path: Path) -> None:
    chain = _create_chain(tmp_path)
    assert not get_unauthorized_widening_deltas(chain, "run-1")


def test_get_unauthorized_widening_deltas_returns_unauthorized(tmp_path: Path) -> None:
    chain = _create_chain(tmp_path)
    record_capability_delta(
        chain=chain,
        run_id="run-1",
    ...
    unauth = get_unauthorized_widening_deltas(chain, "run-1")
    assert unauth == {_DELTA_HASH}


def test_get_unauthorized_widening_deltas_ignores_narrowing(tmp_path: Path) -> None:
    chain = _create_chain(tmp_path)
    record_capability_delta(
        chain=chain,
        run_id="run-1",
...
def test_get_unauthorized_widening_deltas_returns_empty_when_authorized(tmp_path: Path) -> None:
    chain = _create_chain(tmp_path)
    record_capability_delta(
        chain=chain,
        run_id="run-1",
...
def test_get_unauthorized_widening_deltas_rejects_byte_flip(tmp_path: Path) -> None:
    """Byte-flipping the authorization entry makes the widening unverifiable."""
    chain = _create_chain(tmp_path)
    record_capability_delta(
        chain=chain,
        run_id="run-1",
    ...
    # Sanity check: initially it is authorized
    assert not get_unauthorized_widening_deltas(chain, "run-1")

    # Tamper with the authorization event on disk
    target = sorted(chain._log._audit_dir.glob("*.jsonl"))[0]  # pyright: ignore[reportPrivateUsage]
    raw = target.read_bytes()

    needle = b'"authorizer": "operator"'
    offset = raw.find(needle)
    assert offset > 0, "test setup: could not locate authorizer in the record bytes"

    mutated = bytearray(raw)
    char_offset = offset + len(b'"authorizer": "operator')
    mutated[char_offset] ^= 0x01  # Flip a byte in the authorizer field
    target.write_bytes(bytes(mutated))

    # The tamper prevents the hash from matching, so the authorization is rejected
    unauth = get_unauthorized_widening_deltas(chain, "run-1")
    assert unauth == {_DELTA_HASH}, "tampered authorization must be rejected"
