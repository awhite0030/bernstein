...
from bernstein.core.planning.plan_rendering import PlanRendering, render_plan_hash
...
def test_same_inputs_produce_same_hash() -> None:
    plan = _make_plan()
    r1 = render_plan_hash(plan)
    r2 = render_plan_hash(plan)
...
def test_hash_is_64_hex_chars() -> None:
    plan = _make_plan()
    rendering = render_plan_hash(plan)
...
def test_task_order_does_not_affect_hash() -> None:
    ...
    r_fwd = render_plan_hash(plan_fwd)
    r_rev = render_plan_hash(plan_rev)
...
def test_different_journal_head_produces_different_hash() -> None:
    plan = _make_plan()
    r1 = render_plan_hash(plan, journal_head="aaa")
    r2 = render_plan_hash(plan, journal_head="bbb")
    assert r1.rendering_hash != r2.rendering_hash
...
def test_no_journal_head_omits_line() -> None:
    plan = _make_plan()
    rendering = render_plan_hash(plan, journal_head=None)
...
def test_with_journal_head_appears_in_text() -> None:
    plan = _make_plan()
    rendering = render_plan_hash(plan, journal_head="deadbeef")
...
def test_empty_plan_no_estimates() -> None:
    ...
    assert rendering.rendering_hash == render_plan_hash(plan).rendering_hash


def test_empty_plan_with_journal_head() -> None:
    plan = _make_plan(estimates=[], total_cost=0.0, total_minutes=0, high_risk=[])
    r1 = render_plan_hash(plan, journal_head="h1")
    r2 = render_plan_hash(plan, journal_head=None)
    assert r1.rendering_hash != r2.rendering_hash
...
def test_roundtrip_to_dict_from_dict() -> None:
    plan = _make_plan()
    rendering = render_plan_hash(plan, journal_head="abc")
    d = rendering.to_dict()
